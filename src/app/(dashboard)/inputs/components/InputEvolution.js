...
export default function InputEvolution({ inputs, onEdit }) {
  ...
  return (
    <div className="mb-12">
      <h2 className="text-lg font-semibold text-gray-900 mb-5">Input Evolution</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {overperforming.map(inp => (
          <div key={inp.id} className="bg-white rounded-xl shadow-sm border border-green-100 p-5 relative overflow-hidden">
            ...
            <h3 className="font-semibold text-gray-900 text-sm mb-1">{inp.name}</h3>
            <p className="text-sm text-gray-500 mb-4 max-w-[90%]">You have consistently exceeded the target. Consider upgrading it.</p>
            <div className="flex items-center gap-3 text-xs text-gray-600 font-medium mb-4">
              <div className="bg-green-50 text-green-700 px-2.5 py-1 rounded-md">Target: {inp.target}</div>
              <div className="bg-gray-50 text-gray-700 px-2.5 py-1 rounded-md">Actual: {inp.actual}</div>
            </div>
        ...
        {underperforming.map(inp => (
          <div key={inp.id} className="bg-white rounded-xl shadow-sm border border-red-100 p-5 relative overflow-hidden">
            ...
            <h3 className="font-semibold text-gray-900 text-sm mb-1">{inp.name}</h3>
            <p className="text-sm text-gray-500 mb-4 max-w-[90%]">Consistent friction detected. Investigate why this is failing.</p>
            <div className="flex items-center gap-3 text-xs text-gray-600 font-medium mb-4">
